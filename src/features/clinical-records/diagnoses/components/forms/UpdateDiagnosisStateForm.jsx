import { useForm } from 'react-hook-form';
import { Grid } from '@mui/material';
import { useState } from 'react';

import { RHFRadioInput } from '@/components/forms/inputs/index';
import { FormDialog } from '@/components/dialogs/index';
import { handleApiError } from '@/utils/handle-errors';

import { DIAGNOSIS_STATE_LABELS } from '@diagnoses/utils/chip-values';

import { useDiagnoses } from '@diagnoses/hooks/useDiagnoses';
import { updateDiagnosisState } from '@diagnoses/api/diagnosis-api';

export default function UpdateDiagnosisStateForm({ diagnosis, handleClose }) {
    const {
        register,
        handleSubmit,
        control,
        setError: setFormError,
        formState: { errors },
    } = useForm({ mode: 'onBlur', defaultValues: { state: '' } });
    const { refetch } = useDiagnoses();
    const [error, setError] = useState(null);

    const onSubmit = async (data) => {
        try {
            await updateDiagnosisState(diagnosis.uuid, data.state);
            refetch();
            handleClose();
        } catch (err) {
            handleApiError(err, setError, setFormError);
        }
    };

    const statusOptions = Object.entries(DIAGNOSIS_STATE_LABELS).map(([value, label]) => ({ value, label }));

    return (
        <FormDialog
            open={!!diagnosis}
            handleClose={handleClose}
            handleSubmit={handleSubmit(onSubmit)}
            error={error}
            onErrorClose={() => setError(null)}
            title={`Actualizar estado clínico — ${!!diagnosis && diagnosis.name} (${!!diagnosis && diagnosis.patient.fullName})`}
        >
            <Grid container columnSpacing={3} sx={{ pt: 1 }}>
                <Grid size={12}>
                    <RHFRadioInput
                        name='state'
                        control={control}
                        rules={{ required: 'Es obligatorio indicar el nuevo estado' }}
                        errors={errors}
                        label='Nuevo estado'
                        options={statusOptions}
                        row={true}
                    />
                </Grid>
            </Grid>
        </FormDialog>
    );
}
