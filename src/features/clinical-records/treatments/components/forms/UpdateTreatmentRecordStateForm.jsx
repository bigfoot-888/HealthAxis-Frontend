...
import { updateTreatmentRecordState } from '@treatments/api/treatment-api';
import { RHFRadioInput } from '@/components/forms/inputs/index';
import { useState } from 'react';
import { TREATMENT_RECORD_STATE_LABELS } from '@treatments/utils/chip-values';
import { handleApiError } from '@/utils/handle-errors';

export default function UpdateTreatmentRecordStateForm({ treatment, handleClose }) {
    const {
        register,
        handleSubmit,
        control,
    ...
    const onSubmit = async (data) => {
        try {
            await updateTreatmentRecordState(treatment.uuid, data.state);
            refetch();
            handleClose();
        } catch (err) {
    ...
    const statusOptions = Object.entries(TREATMENT_RECORD_STATE_LABELS).map(([value, label]) => ({ value, label }));
